...
// ============ FUNCIONES DE CONTROL DE IVA ============

function loadControlIVA() {
    const tallerId = document.getElementById('filterIVATaller')?.value;
    const fechaInicio = document.getElementById('filterIVAFechaInicio')?.value;
    const fechaFin = document.getElementById('filterIVAFechaFin')?.value;
    const tipoId = document.getElementById('filterIVATipo')?.value;
    
    // Filtrar apuntes según criterios
    let apuntesFiltrados = apuntes;
    
    if (tallerId) {
        apuntesFiltrados = apuntesFiltrados.filter(a => a.tallerId == tallerId);
    }
    
    if (fechaInicio) {
        apuntesFiltrados = apuntesFiltrados.filter(a => a.fecha >= fechaInicio);
    }
    
    if (fechaFin) {
        apuntesFiltrados = apuntesFiltrados.filter(a => a.fecha <= fechaFin);
    }
    
    if (tipoId) {
        apuntesFiltrados = apuntesFiltrados.filter(a => a.tipoId == tipoId);
    }
    ...
    const ivaHoy = apuntes.filter(a => a.fecha === hoy).reduce((sum, a) => sum + a.importe, 0);
    const ivaMes = apuntes.filter(a => a.fecha >= primerDiaMes).reduce((sum, a) => sum + a.importe, 0);
    const ivaCobrado = apuntes.filter(a => a.cobrado).reduce((sum, a) => sum + a.importe, 0);
    const ivaPendiente = apuntes.filter(a => !a.cobrado).reduce((sum, a) => sum + a.importe, 0);
    
    // Actualizar métricas
    const ivaHoyTotal = document.getElementById('ivaHoyTotal');
    const ivaMesTotal = document.getElementById('ivaMesTotal');
    const ivaCobradoTotal = document.getElementById('ivaCobradoTotal');
    const ivaPendienteTotal = document.getElementById('ivaPendienteTotal');
    
    if (ivaHoyTotal) ivaHoyTotal.textContent = `€${ivaHoy.toFixed(2)}`;
    if (ivaMesTotal) ivaMesTotal.textContent = `€${ivaMes.toFixed(2)}`;
    if (ivaCobradoTotal) ivaCobradoTotal.textContent = `€${ivaCobrado.toFixed(2)}`;
    if (ivaPendienteTotal) ivaPendienteTotal.textContent = `€${ivaPendiente.toFixed(2)}`;
    
    // Cargar tabla resumen por taller
    cargarTablaIVATaller(apuntesFiltrados);
    
    // Cargar tabla de detalle
    cargarTablaDetalleIVA(apuntesFiltrados);
    
    // Cargar tabla resumen por tipo
    cargarTablaIVATipo(apuntesFiltrados);
}

function cargarTablaIVATaller(apuntesFiltrados) {
    const tabla = document.getElementById('tablaIVATaller');
    if (!tabla) return;
    ...
    const resumenTalleres = talleres.map(taller => {
        const apuntesTaller = apuntesFiltrados.filter(a => a.tallerId === taller.id);
        const totalIVA = apuntesTaller.reduce((sum, a) => sum + a.importe, 0);
        const cobrado = apuntesTaller.filter(a => a.cobrado).reduce((sum, a) => sum + a.importe, 0);
        const pendiente = apuntesTaller.filter(a => !a.cobrado).reduce((sum, a) => sum + a.importe, 0);
        const numApuntes = apuntesTaller.length;
        const porcentajeCobrado = totalIVA > 0 ? (cobrado / totalIVA * 100).toFixed(1) : 0;
    ...
    }).filter(r => r.numApuntes > 0);
    
    tbody.innerHTML = resumenTalleres.map(r => `
        <tr>
            <td>${r.taller.nombre}</td>
            <td>€${r.totalIVA.toFixed(2)}</td>
            <td style="color: green;">€${r.cobrado.toFixed(2)}</td>
            <td style="color: red;">€${r.pendiente.toFixed(2)}</td>
            <td>${r.numApuntes}</td>
            <td>${r.porcentajeCobrado}%</td>
        </tr>
    `).join('');
}

function cargarTablaDetalleIVA(apuntesFiltrados) {
    const tabla = document.getElementById('tablaDetalleIVA');
    if (!tabla) return;
    
    const tbody = tabla.querySelector('tbody');
    if (!tbody) return;
    
    tbody.innerHTML = apuntesFiltrados.map(apunte => {
        ...
        return `
            <tr>
                ...
                <td>${apunte.facturaALTAI || '-'}</td>
                <td><span style="color: ${apunte.cobrado ? 'green' : 'red'}; font-weight: bold;">${apunte.cobrado ? 'SÍ' : 'NO'}</span></td>
            </tr>
        `;
    }).join('');
}

function cargarTablaIVATipo(apuntesFiltrados) {
    const tabla = document.getElementById('tablaIVATipo');
    if (!tabla) return;
    
    const tbody = tabla.querySelector('tbody');
    if (!tbody) return;
    
    const totalGeneral = apuntesFiltrados.reduce((sum, a) => sum + a.importe, 0);
    
    // Agrupar por tipo
    const resumenTipos = tiposApunte.map(tipo => {
        const apuntesTipo = apuntesFiltrados.filter(a => a.tipoId === tipo.id);
        const totalIVA = apuntesTipo.reduce((sum, a) => sum + a.importe, 0);
        const numApuntes = apuntesTipo.length;
        const porcentaje = totalGeneral > 0 ? (totalIVA / totalGeneral * 100).toFixed(1) : 0;
    ...
    tbody.innerHTML = resumenTipos.map(r => `
        <tr>
            <td><span style="display: inline-block; width: 12px; height: 12px; background-color: ${r.tipo.color}; border-radius: 2px; margin-right: 8px;"></span>${r.tipo.nombre}</td>
            <td>€${r.totalIVA.toFixed(2)}</td>
            <td>${r.numApuntes}</td>
            <td>${r.porcentaje}%</td>
        </tr>
...
function cargarSelectoresControlIVA() {
    // Cargar selector de talleres
    const selectTaller = document.getElementById('filterIVATaller');
    if (selectTaller) {
        selectTaller.innerHTML = '<option value="">-- Todos los talleres --</option>';
        talleres.forEach(taller => {
            const option = document.createElement('option');
    ...
    // Cargar selector de tipos
    const selectTipo = document.getElementById('filterIVATipo');
    if (selectTipo) {
    ...
    const inputInicio = document.getElementById('filterIVAFechaInicio');
    const inputFin = document.getElementById('filterIVAFechaFin');
    
    if (inputInicio) inputInicio.value = primerDia.toISOString().split('T')[0];
    if (inputFin) inputFin.value = hoy.toISOString().split('T')[0];
}

function exportarControlIVA() {
    const tallerId = document.getElementById('filterIVATaller')?.value;
    const fechaInicio = document.getElementById('filterIVAFechaInicio')?.value;
    const fechaFin = document.getElementById('filterIVAFechaFin')?.value;
    const tipoId = document.getElementById('filterIVATipo')?.value;
    ...
    // Filtrar apuntes
    let apuntesExportar = apuntes;
    
    if (tallerId) apuntesExportar = apuntesExportar.filter(a => a.tallerId == tallerId);
    if (fechaInicio) apuntesExportar = apuntesExportar.filter(a => a.fecha >= fechaInicio);
    if (fechaFin) apuntesExportar = apuntesExportar.filter(a => a.fecha <= fechaFin);
    if (tipoId) apuntesExportar = apuntesExportar.filter(a => a.tipoId == tipoId);
    
    // Generar CSV
    let csv = 'Fecha,Taller,ID,Tipo,Cliente,Importe IVA,Método Pago,Factura ALTAI,Cobrado,Notas\n';
    ...
    apuntesExportar.forEach(apunte => {
        ...
        csv += `${apunte.fecha},"${taller.nombre}",${apunte.id},"${tipo.nombre}","${cliente.nombre}",${apunte.importe},${apunte.metodoPago},"${apunte.facturaALTAI || ''}",${apunte.cobrado ? 'SÍ' : 'NO'},"${apunte.notas}"\n`;
    });
    ...
    a.href = url;
    a.download = `control_iva_${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
